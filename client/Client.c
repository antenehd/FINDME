...
int main(){
	...
	char location[101] = "", query[50] = "",answer[255] = "", update[70] = "", name[20] = "", email[25] = "";
	struct info arr[6];
	socklen_t temp_size;
        char temp[100]  = {0};
	...
	printf("1) QUERY with NAME\n");
	printf("2) QUERY with EMAIL\n");
	printf("3) UPDATE\n");
	printf("4) EXIT\n");
		...
		if(option == 1){
			...
			printf("Sent Message: %s", location);
			...
			temp_size = sizeof(serv);
                        memset(answer, 0 ,250);
/*
			if(recvfrom(sokk, answer, 250, 0, (struct sockaddr *) &serv, &temp_size) == -1){
				perror("some error happened at reading the answer of the query\n");
				return -1;
			}
