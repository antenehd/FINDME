...
int main(){
	...
	if(idfile == NULL){
		...
		if(strlen(location) != sendto(sokk, location, strlen(location), 0, (struct sockaddr *) &serv, sizeof(serv))){
			perror("some error happened when sending zero as ID, the location information could not be sent properly:");
			return -1;
		}

		// read from the server what ID it assigns to this client and save it into a file
		temp_size = sizeof(serv);
		if(recvfrom(sokk, &id, sizeof(int), 0, (struct sockaddr*)&serv, &temp_size) == -1){	// modify this in order to decode properly the answer from server, put in select
	...
	temp_size = sizeof(serv);
	if(recvfrom(sokk, answer, (sizeof(char)*249), 0, (struct sockaddr *) &serv, &temp_size) == -1){
