...
int create_list(struct info *list, int elements, char *nombre, char *correo){
	...
	strcpy(list[0].coordenates, "48d51m12sN,2d20m55sE");
	strcpy(list[0].postal, "Paris,France");

	strcpy(list[1].coordenates, "60d10m15sN,24d56m15sE");
	strcpy(list[1].postal, "Helsinki,Finland");

	strcpy(list[2].coordenates, "18d58m39sN,72d49m33sE");
	strcpy(list[2].postal, "Mumbai,India");

	strcpy(list[3].coordenates, "58d18m1sN,134d25m2sW");
	strcpy(list[3].postal, "Juneau,Alaska");

	strcpy(list[4].coordenates, "35d41m5sN,139d41m4sE");
	strcpy(list[4].postal, "Tokio,Japan");

	strcpy(list[5].coordenates, "23d33m3sS,46d38m7sW");
	strcpy(list[5].postal, "Rio de Janeiro,Brasil");

	strcpy(list[6].coordenates, "39d55m9sN,116d23m9sE");
	strcpy(list[6].postal, "Beijing,China");	

	strcpy(list[7].coordenates, "18d56m19sS,47d31m17sE");
	strcpy(list[7].postal, "Antananarivo,Madagascar");

	strcpy(list[8].coordenates, "33d52m10sS,151d12m30sE");
	strcpy(list[8].postal, "Sidney,Australia");

	strcpy(list[9].coordenates, "30d3m1sN,31d14m1sE");
	strcpy(list[9].postal, "Cairo,Egypt");
	...
	// This part separates the answer from the server
		...
		while(save[x] != NULL){
			...
			if((save = realloc(save, sizeof(char*)*(x+1))) == NULL){
				perror("Some error happened when calling malloc:");
				return -1;
			}
			save[x] = strtok(NULL, "$");
		}
...
int main(){
	...
	if(create_list(arr, 10, "nombre", "correo_usuario") != 0){
...
//		if(fread(&id, sizeof(int), 1, idfile) != 1){
		...
		printf("The email read from the file is: %s\n", email);
...
/	leer el nombre y el correo del archivo
