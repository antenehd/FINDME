...
int insert_name_email(struct info *array, const char *nombre, const char *correo, int elements){
	for(int x=0; x < elements; x++){
		memset(array[x].name, 0, (sizeof(char)*40));
		memset(array[x].email, 0, (sizeof(char)*60));
	...
	if(how_many == 3){
		...
		while(save[x] != NULL){
			...
			if((save = realloc(save, sizeof(char*)*(x+1))) == NULL){
				perror("Some error happened when calling realloc in if(how_many == 3):");
				return -1;
			}
			save[x] = strtok(NULL, ":");
		}
...
int build_location(char *locstr, struct info extract, int num_cl, int indic, int option, char *query, char *query2){
	...
	if(!num_cl){
		...
		// this part is when user requests either a query or an update
			...
			// build a string for a requested update, where user inputs updated name and/or email
			else if(option == 3){
				char temp[10] = "";
				sprintf(locstr, "%d$", num_cl);
				strcat(locstr, "UPDATE$NAME$");
				printf("Aqui todavia no hay seg fault, linea 179\n");
				printf("The string contained in save[0] is :%s:\n", query);
				strcat(locstr, query);
				strcat(locstr, "$EMAIL$");
				printf("The string contained in save[1] is :%s:\n", query2);
...
int main(){
	...
	if(create_list(arr, 10, "Test_Name", "Test_Email") != 0){
	...
	// save the beginning of the <save> array of strings
	char **save_beg = save;
	...
	for( ; ; ){
		...
		if(option == 1){
			...
			printf("\nPlease write the updated information in the format <name:email> and press enter\n");
			scanf("%s", update);

			// check if the name contains invalid character '$'
			while(strchr(update, '$') != NULL){
				printf("The word you provided includes the prohibited character $, please write a valid word without $\n");
				scanf("%s", update);
			}
			...
			save = save_beg;
			if(separate_function(update, save, 3) == -1){
				printf("An error happened when calling the separate_function() for option 3 - update info\n");
				return -1;
			}

			// build the update string with the new name or email
			save = save_beg;
			char *query1_point = *save;
			char *query2_point = query1_point;
			query2_point++;
			if(build_location(location, arr[ran_loc], id, indic, option, query1_point, query2_point) != 0){
...
/	the update message when requested by user, after input of the new information, a segfault error is shown, something wrong with the separate fields function
