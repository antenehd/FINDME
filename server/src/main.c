...
int32 CreateMsgQueue()
{
  ...
  mqstat.mq_flags = 0;
   ...
   queue = mq_open(gstConfigs.achMQname,O_CREAT|O_RDWR, MQ_MODE, &mqstat);
...
/*Create socket*/
if(0 > (gUDPCliSockFD = CreateUDPSock(gstConfigs.ui32CPort)))
{
...
if(0 > (gUDPServSockFD = CreateUDPSock(gstConfigs.ui32SPort)))
{
    /*socket creation failed*/
